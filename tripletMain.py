...
def trainModelTriplet(model, n_epochs, optimizer, scheduler, trainLoader, validLoader, n_class, log_batch):
    ...
    for epoch in range(20):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainLoader):
            ...
            loss, fraction = batch_all_triplet_loss(labels, outputs, 1.0)
            # print(loss)
            # stop
            loss.backward()
            optimizer.step()
            ...
            if (i + 1) % log_batch == 0:    # print every 200 mini-batches
                ...
                remain = 50 - percent
                sys.stdout.write("\r[{0}] {1}% loss: {2: 3f} {3}".format('#'*percent + '-'*remain, percent * 2, np.mean(running_loss), total_fraction / log_batch))
                sys.stdout.flush()
...
if __name__ == "__main__":
	data = get_data("VIN")
	datas, targets, data_indexes = preprocessData(data, augment = False)
	datas.shape
	print(datas.shape)
	...
	listModelName = []
	embedding = chooseModel("CNN_LSTM", num_class, input_size = datas[0].shape)
	...
	_, llos = trainModelTriplet(model, n_epochs, optimizer, scheduler, trainLoader, validLoader, 
		n_class= num_class, log_batch=len(trainLoader) / 10)
